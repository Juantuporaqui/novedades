...
window.eliminarJuzgado = async (docid) => {
  ...
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("juzgados").doc(docid).delete();
  cargarListadoJuzgados();
};
...
window.eliminarInhibicion = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("inhibiciones").doc(docid).delete();
  cargarListadoInhibiciones();
};
...
window.eliminarHistoricoJuzgado = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("historicoJuzgados").doc(docid).delete();
  cargarListadoHistoricoJuzgados();
};
...
window.eliminarIntervencion = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("intervenciones").doc(docid).delete();
  cargarListadoIntervenciones();
};
...
window.eliminarSolicitudJudicial = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("solicitudesJudiciales").doc(docid).delete();
  cargarListadoSolicitudesJudiciales();
};
...
window.eliminarColaboracion = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("colaboraciones").doc(docid).delete();
  cargarListadoColaboraciones();
};
...
window.eliminarObservacion = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("observaciones").doc(docid).delete();
  cargarListadoObservaciones();
};
...
window.eliminarPendiente = async (docid) => {
  if(!idOperacionActual) return;
  await db.collection("grupo3_operaciones").doc(idOperacionActual).collection("pendientes").doc(docid).delete();
  cargarListadoPendientes();
};
...
btnAñadirDocumento.addEventListener('click', async ()=>{
  ...
  for(let i=0; i<files.length; i++) {
    ...
    const ref = storage.ref().child(`grupo3/${idOperacionActual}/${file.name}`);
    await ref.put(file);
...
window.eliminarDocumento = async (docid, nombre) => {
  if(!idOperacionActual) return;
  try { await storage.ref().child(`grupo3/${idOperacionActual}/${nombre}`).delete(); } catch(e){}
