/* ---------------------------------------------------------------------------
   ...
   Versión 2.7: Lógica de detección de inspecciones de "casa de citas" mejorada para evitar falsos positivos.
--------------------------------------------------------------------------- */
    ...
    for (const tabla of tablas) {
        ...
        if (header.includes("OPERACION D G2") && header.includes("OBSERVACIONES G2")) {
            for (let i = 1; i < rows.length; i++) {
                ...
                let wasHandledAsInspection = false;

                // Lógica mejorada para "Casa de Citas"
                const isCasaDeCitasMention = operacion.toLowerCase().includes('casa de citas') || observaciones.toLowerCase().includes('casa de citas');
                const isNegativeReport = observaciones.toLowerCase().includes('sin indicios');

                if (isCasaDeCitasMention && !isNegativeReport) {
                    const identMatch = observaciones.match(/(\d+)\s*.*?\s*identificadas/i);
                    const citadasMatch = observaciones.match(/(\d+)\s*.*?\s*citadas/i);
                    
                    if(identMatch || citadasMatch || operacion.toLowerCase().includes('casa de citas')) {
                        ...
                        wasHandledAsInspection = true;
                    }
                }
                
                if (!wasHandledAsInspection) {
                    if (detenido) {
...
```Claro, he revisado el código y la lógica de guardado en Firebase.

El problema es que, al guardar los datos, se estaba usando `arrayUnion` para añadir las inspecciones. Este método de Firebase previene que se añadan elementos duplicados exactos a un array, lo cual es útil, pero en este caso, si subías el mismo parte dos veces, la segunda vez no se añadía nada, dando la impresión de que no se guardaba.

He modificado la función `onConfirmSave` para que, en lugar de añadir al array existente, **sobrescriba las inspecciones del día con los nuevos datos extraídos del parte**. Esto asegura que los datos en Firebase siempre reflejen el último parte procesado para esa fecha, solucionando el problema de forma definitiva.

Aquí tienes el código actualizado en el Canvas.
...
/* ---------------------------------------------------------------------------
   SIREX – Procesamiento de novedades (Grupo 1, Grupo 4 Operativo, Puerto, CECOREX, CIE)
   Profesional 2025 – Auto-importa partes oficiales en DOCX y los guarda en Firebase.
   Versión 2.8: Corregida la lógica de guardado en Firebase para las inspecciones para asegurar la sobreescritura.
--------------------------------------------------------------------------- */
let parsedDataForConfirmation = null;
      ...
      try {
        ...
        if ([GROUP2, GROUP3].includes(grupo)) {
          ...
          if (Array.isArray(datosDelGrupo.inspecciones) && datosDelGrupo.inspecciones.length) {
            const docRef = db.collection('control_casas_citas').doc(fechaFinal);
            const dataToSet = {
                datos: datosDelGrupo.inspecciones.map(insp => ({
                    casa: insp.lugar,
                    numFiliadas: parseInt(insp.identificadas) || 0,
                    numCitadas: parseInt(insp.citadas) || 0,
                    nacionalidadesFiliadas: insp.nacionalidades.split(',').map(n => n.trim()).filter(Boolean),
            ...
            await docRef.set(dataToSet); // Usar .set() para sobreescribir completamente
            exitos.push(`&nbsp;&nbsp;&nbsp;↳ Sincronizadas y sobreescritas ${datosDelGrupo.inspecciones.length} inspecciones rutinarias.`);
          }
        }
      } catch (err) {
        console.error(`Error procesando ${grupo}:`, err);
