...
// Corregido para no usar imports de librerías globales.
...
const addPersonaImplicada = (data = {}) => addDynamicItem(document.getElementById('personasImplicadasContainer'),
    [{ idPrefix: 'impNombre', label: 'Nombre', valueField: 'nombre' }, { idPrefix: 'impNac', label: 'Nacionalidad', valueField: 'nacionalidad' }, { idPrefix: 'impFechaExp', label: 'Fecha Expulsión', type: 'date', valueField: 'fechaExpulsion' }], data);
const getPersonasImplicadas = () => getDynamicItems(document.getElementById('personasImplicadasContainer'),
    [{ idPrefix: 'impNombre', valueField: 'nombre' }, { idPrefix: 'impNac', valueField: 'nacionalidad' }, { idPrefix: 'impFechaExp', valueField: 'fechaExpulsion' }]);
const addExpulsado = (data = {}) => addDynamicItem(document.getElementById('expulsadosContainer'),
    [{ idPrefix: 'expNombre', label: 'Nombre', valueField: 'nombre' }, { idPrefix: 'expNac', label: 'Nacionalidad', valueField: 'nacionalidad' }], data);
const getExpulsados = () => getDynamicItems(document.getElementById('expulsadosContainer'),
    [{ idPrefix: 'expNombre', valueField: 'nombre' }, { idPrefix: 'expNac', valueField: 'nacionalidad' }]);
const addFletado = (data = {}) => addDynamicItem(document.getElementById('fletadosContainer'),
    [{ idPrefix: 'fletDestino', label: 'Destino', valueField: 'destino' }, { idPrefix: 'fletPax', label: 'Pax', type: 'number', valueField: 'pax' }], data);
const getFletados = () => getDynamicItems(document.getElementById('fletadosContainer'),
    [{ idPrefix: 'fletDestino', valueField: 'destino' }, { idPrefix: 'fletPax', valueField: 'pax' }]);
const addConduccionPositiva = (data = {}) => addDynamicItem(document.getElementById('conduccionesPositivasContainer'),
    [{ idPrefix: 'cpDesc', label: 'Descripción', valueField: 'descripcion' }], data);
const getConduccionesPositivas = () => getDynamicItems(document.getElementById('conduccionesPositivasContainer'),
    [{ idPrefix: 'cpDesc', valueField: 'descripcion' }]);
const addConduccionNegativa = (data = {}) => addDynamicItem(document.getElementById('conduccionesNegativasContainer'),
    [{ idPrefix: 'cnDesc', label: 'Descripción', valueField: 'descripcion' }], data);
const getConduccionesNegativas = () => getDynamicItems(document.getElementById('conduccionesNegativasContainer'),
    [{ idPrefix: 'cnDesc', valueField: 'descripcion' }]);
...
async function generateGroup1Pdf() {
    ...
    const q = query(collection(db, `artifacts/${getAppId()}/expulsiones`), where('fecha', '>=', start), where('fecha', '<=', end));
    ...
    snaps.docs.forEach(d => {
        ...
        summary[dateKey].fletados += data.fletados?.reduce((acc, f) => acc + (parseInt(f.pax, 10) || 0), 0) || 0;
    ...
    const rows = Object.keys(summary).sort((a, b) => new Date(a.split('-').reverse().join('-')) - new Date(b.split('-').reverse().join('-'))).map(fecha => [fecha, summary[fecha].expulsados, summary[fecha].fletados, summary[fecha].conduccionesPositivas, summary[fecha].conduccionesNegativas]);
    doc.autoTable({
        head: [['Fecha', 'Expulsados', 'Pax Fletados', 'Cond. Positivas', 'Cond. Negativas']],
        body: rows,
        startY: 35,
        theme: 'grid',
...
export const getGrupo1Config = () => ({
    ...
    `,
    dynamicListContainers: ['personasImplicadasContainer', 'expulsadosContainer', 'fletadosContainer', 'conduccionesPositivasContainer', 'conduccionesNegativasContainer', 'grupoPendientesList'],
    dataMap: { anio: 'anio', descripcionBreve: 'descripcionBreve', personasImplicadas: getPersonasImplicadas, expulsados: getExpulsados, fletados: getFletados, conduccionesPositivas: getConduccionesPositivas, conduccionesNegativas: getConduccionesNegativas, grupoPendientes: getGrupoPendientes },
    setupEventListeners: () => {
